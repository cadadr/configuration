...
apt-get update -q || (echo Failed updating apt repositories && exit 1)
apt-get -y install -q make curl apt-transport-https
...
if [ ! -f /etc/apt/sources.list.d/syncthing.list ]; then
    ...
    apt-get update -q ||\
	(echo Failed re-updating apt repositories && exit 1)
fi
...
(
	...
	apt-get -y install -q ./goktug.deb && make ubuntu-config
    fi \
...
if id $username 2>/dev/null >/dev/null; then
    ...
    useradd -c 'Goktug Kayaalp' -d /home/$username \
	 -G sudo,adm,cdrom,dip,plugdev,lpadmin,sambashare -m -s /bin/bash -U -u 1993 $username \
