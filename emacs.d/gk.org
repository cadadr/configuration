...
WikiZero is a mirror of wikipedia."
  ...
  (let* ((baseurl "http://www.wiki-zero.com/index.php?q=")
         (wiki64 (base64-encode-string (url-unhex-string url) t))
         (0url (concat baseurl wiki64)))
    (browse-url 0url)))
       ...
       (unless (process-live-p process)
         (when (zerop (process-exit-status process))
           (dolist (f gk-mail-inboxes)
             (when (> (file-attribute-size (file-attributes f))
                      0)
...
*** Mairix
Mairix is the mail search tool.  =mairix.el= seems to come with Emacs.
...
**** Files and directories
#+BEGIN_SRC elisp :tangle yes :comments both
(setf gnus-default-directory (expand-file-name "old-gnus" gk-mail-home))
...
**** Settings
Modifications to how Gnus behaves.
...
(setf
 ...
 org-src-inherited-local-variables
 '(lexical-binding truncate-lines))
#+END_SRC
...
;; This is how you undo a [remap <fn>] style keybinding. 
...
(defvar gk-after-save-org-idle-seconds 2)
...
Does various tasks after saving a file, see it's definition."
 ...
 timers, will query for export when Emacs is idle for %d seconds"
      ...
      (setf
       ...
       (run-with-idle-timer
        ...
        (lambda ()
          (when (y-or-n-p
                 "You just wrote an Org agenda file, regenerate ICS files now?")
...
(gk-global-binding "\C-l" #'recenter)
...
(gk-prefix-binding (kbd "C-<Return>") #'org-babel-tangle-jump-to-org)
#+END_SRC
