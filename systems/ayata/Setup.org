...
3) Rudimentary setup
   ...
   3) Change hostname
      ...
      echo ayata | sudo tee /etc/hostname
      #+end_src
...
6) Copy over configuration files. This overrites matching files under
   ...
   scp -rv etc pi@ayata.local:$tmp
   ssh pi@ayata.local cd $tmp \; sudo cp -vr \* /etc
   #+end_src

7) Restart system services
   1) All but network:
      #+begin_src sh :dir /ssh:pi@ayata.local:~ :results output
...
#+begin_src sh :dir /ssh:pi@ayata.local:~ :results output :var _TZ=timezone
...
- In order to configure scanner connection, first verify that the
  ...
  #+begin_src sh :dir /ssh:pi@ayata.local:~
  scanimage -L
  #+end_src

  - Append RPi’s FQDN to ~/etc/sane.d/net.conf~.
    #+begin_src sh :dir /sudo::
    echo ayata.local >> /etc/sane.d/net.conf
    #+end_src
  ...
  - Add ~saned@ayata.local~ to the ~lp~ group
    #+begin_src sh :dir /ssh:pi@ayata.local
