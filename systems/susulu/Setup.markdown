# Setup recipe for `susulu`

Install using the KDE Neon User live image.
...
and then enable source repositories.

    $ sed s/^deb/deb-src/ < /etc/apt/sources.list | sudo tee /etc/apt/sources.list.d/src.list
    $ sudo apt-get update
